...
void display(Buffer* ptr_Buffer) {
	...
	//printf("The first symbol in the buffer is:  %c\n", bufferAddCPosition(ptr_Buffer) ? *bufferGetString(ptr_Buffer, 0) : ' ');
...
{
	pBuffer p = malloc(DEFAULT_FLAGS);
	p->capacity = DEFAULT_FLAGS;
	return p;
}
...
char** bufferLoad(pBuffer ptr_Buffer, char* file)
{
	...
	if (Readfile != NULL) {
		...
		if (fseek(Readfile, 0L, SEEK_END) == 0) { /* Go to the end of the file. */

			string = malloc(DEFAULT_FLAGS); /* Allocate buffer to size of DEFAULT_FLAGS. */
			if (fseek(Readfile, 0L, SEEK_SET) != 0) {} /* Go back to the start of the file. */
			size_t newLen = fread(string, sizeof(char), DEFAULT_FLAGS, Readfile); /* Read the entire file into memory. */

			if (ferror(Readfile) != 0) {
				fputs("Error reading file", stderr);
			}
			else { string[newLen++] = '\0'; /* Gets rid of junk characters */ }
		}
		fclose(Readfile);
	}
