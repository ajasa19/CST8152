...
Author: Asim Jasarevic
History/Versions: 1.0, 3 Oct 2020
...
int bufferIsFull(Buffer* const pBE) {
	...
	/*check if buffer exsists if not can't work*/
	if (!pBE) {
		return -1;
	}
...
short bufferGetAddCPosition(Buffer* const pBE) {
	...
	/*check if buffer exsist*/
	if (pBE->addCPosition < 0 || pBE->addCPosition > SHRT_MAX) {
		return -1;
	}
...
short bufferGetCapacity(Buffer* const pBE) {
	...
	/*check if buffer capacity exsists if not can't return value*/
	if (!pBE || pBE->capacity > SHRT_MAX || pBE->capacity < 0) {
		return -1;
	}
...
int bufferGetOpMode(Buffer* const pBE) {
	...
	/*check if buffer opMode exsists if not can't return value*/
	if (!pBE || pBE->opMode <-1 || pBE->opMode > 1) {
		return -1;
	}
...
size_t bufferGetIncrement(Buffer* const pBE) {
	...
	/*check if buffer increment exsists if not can't return value
	  check if buffer exsists at all*/
	if (!pBE || pBE->increment < 0 || pBE->increment > SHRT_MAX) {
		return 0x0100;
	}
...
int bufferLoad(FILE* const fi, Buffer* const pBE) {
	...
	/*check if file passed and first char are getting/have values if not can't read*/
	if (!fi || !c || !pBE) {
		return -1;
	}
...
int bufferIsEmpty(Buffer* const pBE) {
	...
	/*check if buffer addCPosition exsists if not can't return value*/
	if (!pBE || pBE->addCPosition < 0 || pBE->addCPosition >=SHRT_MAX) {
		return -1;
	}
...
char bufferGetChar(Buffer* const pBE) {
	...
	/*check if buffer flags exsists if not can't return value
	 check if buffer exsists at all*/
	if (!pBE || !pBE->flags) {
		return -2;
	}
...
int bufferPrint(Buffer* const pBE, char newLine) {
	...
	/*check if buffer flags exsists/newLine if not can't return value
     check if buffer exsists at all*/
	if (!pBE || !pBE->flags || !newLine) {
		return -1;
	}
...
Buffer* bufferSetEnd(Buffer* const pBE, char symbol) {
	...
	/*check if buffer vars exsists if not can't execute equation
  check if buffer exsists at all*/
	if (!pBE || !symbol || pBE->capacity < 0 || pBE->capacity >= SHRT_MAX || !pBE->string || pBE->addCPosition < 0 || pBE->addCPosition >= SHRT_MAX) {
		return NULL;
	}
...
short bufferReset(Buffer* const pBE) {
	...
	/*check if buffer vars exsists if not can't execute equation
    check if buffer exsists at all*/
	if (!pBE || pBE->markCPosition < 0 || pBE->markCPosition > SHRT_MAX || pBE->getCPosition < 0 || pBE->getCPosition >= SHRT_MAX) {
		return -1;
	}
...
int bufferRewind(Buffer* const pBE) {
	...
	/*check if two vlues where set to 0*/
	if (pBE->getCPosition != 0 || pBE->markCPosition != 0) {
		return -1;
	}
...
char* bufferGetString(Buffer* const pBE, short charPosition) {
	...
	/*check if buffer charPosition is not 0 if not can't get right value
	  check if buffer exsists at all*/
	if (!pBE  || charPosition != 0) {
		return NULL;
	}
